...
impl feo_demo::Guest for Component {
    fn configure_demo(config: feo_demo::DemoConfig) -> Result<(), String> {
        unsafe {
            ...
            println!(
                "FEO Demo configured: mode={:?}, max_cycles={}, delay={}ms",
        ...
        unsafe {
            if let Some(ref mut state) = DEMO_STATE {
                ...
                if crate::video_decoder_feo::has_output() {
                    ...
                    match crate::object_detection_feo::execute_cycle() {
                        Ok(metrics) => {
                            ...
                            if crate::object_detection_feo::has_output() {
                                results
                                    .push("✓ Object detections ready for next stage".to_string());
                            }
                        }
                        Err(e) => {
                    ...
                    results
                        .push("⚠ Skipping object detection - no input frame available".to_string());
                }
    ...
    fn execute_n_cycles(count: u32) -> Result<String, String> {
        ...
        unsafe {
            if let Some(ref state) = DEMO_STATE {
                ...
                all_results.push(format!(
                    "Video decoder executions: {}",
                    state.video_executions
                ));
                all_results.push(format!(
                    "Object detection executions: {}",
                    state.detection_executions
                ));
                all_results.push(format!(
                    "Average cycle time: {:.1} μs",
                    state.total_execution_time as f64 / state.total_cycles as f64
    ...
    fn get_component_status() -> Result<String, String> {
        ...
        status.push(format!(
            "Can Execute: {}",
            crate::video_decoder_feo::can_execute()
        ));
        status.push(format!(
            "Has Output: {}",
            crate::video_decoder_feo::has_output()
        ));
        ...
        status.push(format!(
            "Execution Budget: {} μs, Memory Budget: {} bytes",
        ...
        status.push(format!(
            "Can Execute: {}",
            crate::object_detection_feo::can_execute()
        ));
        status.push(format!(
            "Has Output: {}",
            crate::object_detection_feo::has_output()
        ));
    ...
    fn run_full_diagnostics() -> Result<String, String> {
        ...
        match crate::object_detection_feo::get_diagnostics() {
            Ok(diag) => diagnostics.push(diag),
            Err(e) => {
                diagnostics.push(format!("Error getting object detection diagnostics: {}", e))
            }
        }
        ...
        unsafe {
            if let Some(ref state) = DEMO_STATE {
                diagnostics.push(format!(
                    "Demo Configuration: {:?}",
                    state.config.as_ref().map(|c| c.mode)
                ));
