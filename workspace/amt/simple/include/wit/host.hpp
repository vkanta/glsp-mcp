...
public:
#ifdef WIT_HOST_WAMR
  string_view_impl::string_view get_view(WASMExecEnv *inst) const {
    return string_view_impl::string_view((char const *)wasm_runtime_addr_app_to_native(
                                wasm_runtime_get_module_inst(inst), data_),
                            length);
  }
#elif defined(WIT_HOST_DIRECT)
string_view_impl::string_view get_view() const {
    return string_view_impl::string_view((char const *)data_, length);
  }
#endif
  ...
  static string from_view(string_view_impl::string_view v) {
    void *addr = cabi_realloc(nullptr, 0, 1, v.size());
    memcpy(addr, v.data(), v.size());
    return string((guest_address)addr, v.size());
  }
#endif
#if defined(WIT_HOST_WAMR)
  static string from_view(wasm_exec_env_t exec_env, string_view_impl::string_view v) {
    void *addr = nullptr;
...
public:
#ifdef WIT_HOST_WAMR
  span_impl::span<T> get_view(WASMExecEnv *inst) const {
    return span_impl::span((T const *)wasm_runtime_addr_app_to_native(
                         wasm_runtime_get_module_inst(inst), data_),
                     length);
  }
#elif defined(WIT_HOST_DIRECT)
...
#if defined(WIT_HOST_DIRECT)
  static vector from_view(span_impl::span<T> v) {
    void *addr = cabi_realloc(nullptr, 0, sizeof(T), v.size());
    memcpy(addr, v.data(), sizeof(T) * v.size());
    return vector((guest_address)addr, v.size());
  }
#endif
};
...
template <class R> class ResourceForwarder : public R {
  ...
  optional_impl::optional<Owned> lookup_resource(int32_t id) {
    // TODO: Handle not found
    return R(ResourceExportBase(id));
  }
  optional_impl::optional<Owned> remove_resource(int32_t id) {
    optional_impl::optional<R *> result = R::remove_resource(id);
    if (!result.has_value())
      return optional_impl::optional<Owned>();
    return *result;
  }
};
} // namespace wit
