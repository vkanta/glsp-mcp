...
use crate::wasm::pipeline::{WasmPipelineEngine, PipelineConfig};
use crate::wasm::sensor_bridge::{SensorDataBridge, SensorBridgeConfig, BridgeStatus};
...
pub enum TriggerCondition {
    ...
    SensorValue { sensor_id: String, condition: FilterCondition },
...
impl WasmSimulationEngine {
    ...
    pub async fn execute_simulation(&self, config: SimulationConfig) -> Result<String> {
        ...
        let sensor_bridge = if let Some(ref dataset_manager) = self.dataset_manager {
            match SensorDataBridge::new(config.sensor_config.clone(), Some(dataset_manager.clone())).await {
                Ok(bridge) => {
    ...
    async fn execute_simulation_impl(
        ...
        if let SimulationExecutionMode::RealTime = config.settings.execution_mode {
            ...
            sync.frame_duration = Duration::from_millis(
                (1000.0 / config.settings.real_time_sync.target_fps) as u64
            );
    ...
    async fn execute_real_time_simulation(
        ...
        loop {
            ...
            Self::execute_frame_scenarios(
                pipeline_engine.clone(),
                execution_arc.clone(),
                config,
            ).await?;
            ...
            // Real-time synchronization
            {
                ...
                if sync.is_active {
                    ...
                    if frame_elapsed < sync.frame_duration {
                        ...
                        debug!("Frame took {}ms, target is {}ms", 
    ...
    async fn execute_accelerated_simulation(
        ...
        info!("Executing accelerated simulation: {} ({}x speed)", config.name, speed_multiplier);
        
        // Similar to real-time but with adjusted timing
        let mut _frame_count = 0u64;
        let frame_duration = Duration::from_millis((1000.0 / (config.settings.real_time_sync.target_fps * speed_multiplier)) as u64);
    ...
    async fn execute_step_by_step_simulation(
        ...
        info!("Step-by-step simulation mode not fully implemented for: {}", config.name);
    ...
    async fn execute_batch_simulation(
        ...
        info!("Batch simulation mode not fully implemented for: {}", config.name);
    ...
    async fn execute_frame_scenarios(
        ...
        for scenario in &config.scenarios {
            for pipeline_config in &scenario.pipelines {
                match pipeline_engine.execute_pipeline(pipeline_config.clone()).await {
                    Ok(pipeline_execution_id) => {
                        debug!("Started pipeline: {} -> {}", pipeline_config.name, pipeline_execution_id);
