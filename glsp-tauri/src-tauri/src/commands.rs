...
#[command]
pub async fn get_workspace_info(workspace_path: Option<String>, state: State<'_, AppState>) -> Result<WorkspaceInfo, String> {
    match workspace_path {
        ...
        None => {
            ...
            let workspace_root = workspace_info.get("workspace_root")
                .and_then(|v| v.as_str())
                .unwrap_or("Unknown");
            let _wasm_path = workspace_info.get("wasm_components_path")
                .and_then(|v| v.as_str())
                .unwrap_or("");
            let _diagrams_path = workspace_info.get("diagrams_path")
                .and_then(|v| v.as_str())
                .unwrap_or("");
            let wasm_count = workspace_info.get("wasm_components_count")
                .and_then(|v| v.as_u64())
                .unwrap_or(0) as u32;
            let diagrams_count = workspace_info.get("diagrams_count")
                .and_then(|v| v.as_u64())
                .unwrap_or(0) as u32;
...
pub async fn get_recent_workspaces() -> Result<Vec<WorkspaceInfo>, String> {
    ...
    let recent_workspaces = settings.get("recent_workspaces")
...
pub async fn add_recent_workspace(workspace_path: String) -> Result<(), String> {
    ...
    let recent_workspaces = settings.get_mut("recent_workspaces")
...
#[command]
pub async fn set_workspace_directory(workspace_path: String, create_if_missing: Option<bool>, state: State<'_, AppState>) -> Result<String, String> {
    info!("Setting workspace directory to: {}", workspace_path);
    
    let create = create_if_missing.unwrap_or(true);
    let result = state.mcp_client.set_workspace_directory(&workspace_path, create).await?;
...
#[command]
pub async fn validate_workspace(workspace_path: String, state: State<'_, AppState>) -> Result<serde_json::Value, String> {
...
#[command]
pub async fn create_workspace_structure(workspace_path: String, state: State<'_, AppState>) -> Result<String, String> {
    info!("Creating workspace structure at: {}", workspace_path);
    state.mcp_client.create_workspace_structure(&workspace_path).await
}